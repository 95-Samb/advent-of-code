...
describe NewIntcode do
  ...
  context "for 5 and 6 instruction opcodes" do
    ...
    it "returns [3,1,1105,1,9,1,1,1,12,4,12,99,1],[1] for 1,[3,1,1105,1,9,1,1,1,12,4,12,99,1]" do
      expect(NewIntcode.new.execute([1],[3,1,1105,1,9,1,1,1,12,4,12,99,1])).to eq([[3,1,1105,1,9,1,1,1,12,4,12,99,1],[1]])
    end
  ...
  context "for 7 and 8 instruction opcodes" do
    ...
    it "returns [1107,1,2,5,99,1] for [1107,1,2,5,99,0]" do
      expect(NewIntcode.new.execute([0],[1107,1,2,5,99,0])[0]).to eq([1107,1,2,5,99,1])
    end
    it "returns [1107,1,1,5,99,0] for [1107,1,1,5,99,9]" do
      expect(NewIntcode.new.execute([0],[1107,1,1,5,99,9])[0]).to eq([1107,1,1,5,99,0])
    end
    it "returns [107,6,3,5,99,0] for [107,6,3,5,99,7]" do
      expect(NewIntcode.new.execute([0],[107,6,3,5,99,7])[0]).to eq([107,6,3,5,99,0])
    end
    it "returns 1,[3,9,1007,9,4,9,4,9,99,1] for 3,[3,9,1007,9,4,9,4,9,99,1]" do
      expect(NewIntcode.new.execute([3],[3,9,1007,9,4,9,4,9,99,1])).to eq([[3,9,1007,9,4,9,4,9,99,1],[1]])
    end
    ...
    it "returns [1108,1,1,5,99,1] for [1108,1,1,5,99,0]" do
      expect(NewIntcode.new.execute([0],[1108,1,1,5,99,0])[0]).to eq([1108,1,1,5,99,1])
    end
    it "returns [1108,1,2,5,99,0] for [1108,1,2,5,99,9]" do
      expect(NewIntcode.new.execute([0],[1108,1,2,5,99,9])[0]).to eq([1108,1,2,5,99,0])
    end
  end
  context "for advent of code examples given" do
    it "returns 1 for 8,[3,9,8,9,10,9,4,9,99,-1,8]" do
      expect(NewIntcode.new.execute([8],[3,9,8,9,10,9,4,9,99,-1,8])[1]).to eq([1])
    end
    it "returns 1 for 7,[3,9,7,9,10,9,4,9,99,-1,8]" do
      expect(NewIntcode.new.execute([7],[3,9,7,9,10,9,4,9,99,-1,8])[1]).to eq([1])
    end
    it "returns 1 for 8,[3,3,1108,-1,8,3,4,3,99]" do
      expect(NewIntcode.new.execute([8],[3,3,1108,-1,8,3,4,3,99])[1]).to eq([1])
    end
    it "returns 1 for 7,[3,3,1107,-1,8,3,4,3,99]" do
      expect(NewIntcode.new.execute([7],[3,3,1107,-1,8,3,4,3,99])[1]).to eq([1])
    end
    it "returns 0 for 0,[3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9]" do
      expect(NewIntcode.new.execute([0],[3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9])[1]).to eq([0])
    end
    it "returns 1 for 5,[3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9]" do
      expect(NewIntcode.new.execute([5],[3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9])[1]).to eq([1])
    end
    it "returns 0 for 0,[3,3,1105,-1,9,1101,0,0,12,4,12,99,1]" do
      expect(NewIntcode.new.execute([0],[3,3,1105,-1,9,1101,0,0,12,4,12,99,1])[1]).to eq([0])
    end
    it "returns 1 for 5,[3,3,1105,-1,9,1101,0,0,12,4,12,99,1]" do
      expect(NewIntcode.new.execute([5],[3,3,1105,-1,9,1101,0,0,12,4,12,99,1])[1]).to eq([1])
    end
    it "returns 1000 for 8,[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
      ...
      expect(NewIntcode.new.execute([8],[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
                                      1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
                                      999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99])[1]).to eq([1000])
    end
    it "returns 999 for 7,[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
        1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
        999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99]" do
      expect(NewIntcode.new.execute([7],[3,21,1008,21,8,20,1005,20,22,107,
                                      8,21,20,1006,20,31,1106,0,36,98,
                                      0,0,1002,21,125,20,4,20,1105,1,
                                      46,104,999,1105,1,46,1101,1000,1,20,
                                      4,20,1105,1,46,98,99])[1]).to eq([999])
    end
    it "returns 1001 for 9,[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
        1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
        999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99]" do
      expect(NewIntcode.new.execute([9],[3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
                                      1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
                                      999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99])[1]).to eq([1001])
    end
  end
  context "for multiple 3 instructions" do
    it "returns [3,5,3,6,99,1,1] for [1,1],[3,5,3,6,99,0,0]" do
      expect(NewIntcode.new.execute([1,1],[3,5,3,6,99,0,0])[0]).to eq([3,5,3,6,99,1,1])
    end
    it "returns [1,0,3,6,99,0,1] for [1,1],[3,0,3,6,99,0,0]" do
      expect(NewIntcode.new.execute([1,1],[3,0,3,6,99,0,0])[0]).to eq([1,0,3,6,99,0,1])
    end
  end
end
